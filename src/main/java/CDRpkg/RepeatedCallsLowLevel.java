...
public class RepeatedCallsLowLevel {
...
//        env.setParallelism(1);
        ...
        DataStream<KafkaEventOut> input = env
				...
				.process( new KeyedProcessFunction<Tuple, KafkaEventIn, KafkaEventOut>() {
...
//						valueDescriptor.enableTimeToLive(ttlConfig);
					...
					@Override
					public void processElement(KafkaEventIn value, Context ctx, Collector<KafkaEventOut> out) throws Exception {

						KafkaEventIn iWindow = KafkaEventIn.fromString(value.toString());
						KafkaEventIn iHistory = null;
						KafkaEventOut iOutput = new KafkaEventOut(iWindow);
						...
						ctx.timerService().registerEventTimeTimer(iWindow.getUnixtimestamp() + 25000);


					}

					@Override
					public void onTimer(long timestamp, OnTimerContext ctx, Collector<KafkaEventOut> out) throws Exception {
						if (historyElement.value() != null) {
